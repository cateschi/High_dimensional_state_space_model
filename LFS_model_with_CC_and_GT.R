...
objopt.GT <- ucminf(par=init.val.GT, LFS_CC_GT,y,opti=T,k,outofsample=T,parP10=1000000000000,nfactors,
                    nstates,lambda,H=H,ns.id,hessian=2,control=list(grad="central", gradstep = c(1e-2, 1e-3), trace=T))

par.GT <- objopt.GT$par

obj <- LFS_CC_GT(par=objopt.GT$par,y,opti=F,k,outofsample=T,parP10=1000000000000,nfactors,nstates,lambda,H=H,ns.id)
