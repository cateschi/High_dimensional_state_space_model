...
KF_slopes_mixed_factor <- function(par,y,opti,k,delta,outofsample,parP10,nstates,lambda,H,ns.id,sARIMA){
    len <- length(y[1,])
    sigma_Ry <- par[1]
    sigma_omegay <- par[2]
    sigma_lambda <- par[3]
    ...
    D <- adiag(0, exp(sigma_Ry), exp(sigma_omegay)*diag(11), exp(sigma_lambda)*diag(4), sd_nu, diag(0,8,8), 0, exp(sigma_Rx), 0, exp(sigma_Rx), diag(sqrt(diag(H)[ns.id])))
    gamma <- par[9]
    R[32,2] <- tanh(gamma)
    R[2,32] <- tanh(gamma)
    Q <- D%*%R%*%D
